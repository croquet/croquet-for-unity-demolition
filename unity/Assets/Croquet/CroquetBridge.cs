...
public class CroquetBridge : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        loadingBar = GameObject.Find("LoadingCanvas").GetComponent<LoadingBar>();
    ...
    void StartWS()
    {
        ...
        if (myQRShower != null)
        {
            myQRShower.DisplayQR($"https://croquet.io/demolition-multi/?q={sessionNameValue}");
        }
    ...
    void ProcessCroquetCommand(string msg)
    {
        ...
        // introduced for demolition
        else if (command == "joinProgress") HandleJoinProgress(args[0]);
        else if (command == "fuseLit") BarrelFuseLit(args);
    ...
    {
        // as soon as any object's position is being adjusted, make sure the loading bar is hidden
    ...
    {
        GameObject barrel = FindObject(args[0]);
        if (barrel != null)
        {
    ...
    {
        //Debug.Log($"PROGRESS ${ratio} ${msg} ${smoothed}");
        loadingBar.SetProgress(ratio, msg);
    }
    ...
    void SetLoadingProgress(float loadRatio)
    {
        ...
        loadingBar.SetProgress(barRatio, $"Loading... ({loadRatio * 100:#0.0}%)");
    }
